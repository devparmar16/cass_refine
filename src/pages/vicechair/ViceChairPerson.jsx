...
const EventView = ({ event, onBack, userRole }) => {
  ...
  const fetchTasks = async () => {
    ...
    try {
      // Debug log
      console.log('[ViceChair EventView] Fetching tasks with:', {
        eventId: event?.id,
        userRole,
        tab
      });
      ...
      // Debug log
      console.log('[ViceChair EventView] Query result:', { data, error });
  ...
  return (
    <div>
      <button onClick={onBack} className="mb-4 px-3 py-1 bg-gray-200 rounded hover:bg-gray-300">
        Back to Events
      </button>
...
const ViceChairPerson = () => {
  ...
  const currentUserRole = normalizeRole(user?.role) || 'vice_chairperson';
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold mb-6">Vice Chairperson Events Page</h1>
      {!selectedEvent ? (
